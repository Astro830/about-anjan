...
exports.processRunnerProgress = processRunnerProgress;
...
async function processRunnerProgress(eventId, runnerId) {
    const events = await (0, db_1.runQuery)(`SELECT start_latitude, start_longitude, end_latitude, end_longitude FROM events WHERE id = $1`, [eventId]);
    if (events.length === 0)
        return;
    const ev = events[0];
    const positions = await (0, db_1.runQuery)(`SELECT ts, ST_Y(geom) AS latitude, ST_X(geom) AS longitude
     FROM positions
     WHERE event_id = $1 AND runner_id = $2
     ORDER BY ts ASC`, [eventId, runnerId]);
    ...
    for (let i = 0; i < positions.length; i += 1) {
        const p = positions[i];
        const d = (0, geo_1.haversineDistanceMeters)({ lat: p.latitude, lon: p.longitude }, startCenter);
    ...
    for (let i = startIdx; i < positions.length; i += 1) {
        const p = positions[i];
        const d = (0, geo_1.haversineDistanceMeters)({ lat: p.latitude, lon: p.longitude }, endCenter);
        if (d <= DEFAULT_GEOFENCE_RADIUS_M) {
            finishIdx = i;
            break;
        }
    ...
    for (let i = startIdx + 1; i <= finishIdx; i += 1) {
        ...
        totalMeters += (0, geo_1.haversineDistanceMeters)({ lat: prev.latitude, lon: prev.longitude }, { lat: curr.latitude, lon: curr.longitude });
    }
    const startTime = positions[startIdx].ts;
    const endTime = positions[finishIdx].ts;
    const totalTimeSecRows = await (0, db_1.runQuery)(`SELECT EXTRACT(EPOCH FROM ($2::timestamptz - $1::timestamptz)) AS seconds`, [startTime, endTime]);
    const totalSec = totalTimeSecRows[0]?.seconds ?? 0;
    const paceSecPerKm = totalMeters > 0 ? (totalSec / (totalMeters / 1000)) : 0;
    await (0, db_1.runQuery)(`INSERT INTO results (runner_id, event_id, start_time, end_time, total_distance_m, avg_pace_sec_per_km)
     VALUES ($1,$2,$3,$4,$5,$6)
     ON CONFLICT (runner_id, event_id)
     DO UPDATE SET start_time = EXCLUDED.start_time,
                   end_time = EXCLUDED.end_time,
                   total_distance_m = EXCLUDED.total_distance_m,
                   avg_pace_sec_per_km = EXCLUDED.avg_pace_sec_per_km`, [runnerId, eventId, startTime, endTime, totalMeters, paceSecPerKm]);
    await broadcastLeaderboard(eventId);
}
async function broadcastLeaderboard(eventId) {
    // Combine finished results (rank by time) and in-progress (rank by distance)
    const rows = await (0, db_1.runQuery)(`WITH d AS (
       SELECT rp.runner_id,
              rp.event_id,
              rp.total_distance_m
     ...
     LIMIT 100`, [eventId]);
    (0, leaderboard_1.broadcast)('leaderboard', { eventId, leaderboard: rows });
}
